...
// Loop to get the next 5 dates after today
for (let i = 0; i < 5; i++) {
  const nextDate = new Date(today);
  nextDate.setDate(today.getDate() + i);
  nextFiveDates.push(nextDate.toISOString().split("T")[0]);
}

   // Filtering data to get the first entry after 6 AM for each unique date
   const firstDataForEachDate = nextFiveDates.map(date => {
